...
static int test_sdl_delay_within_bounds(void) {
    ...
    result = SDL_GetTicks();
    SDL_Log("Call to SDL_GetTicks()");
    SDL_Log(result > 0, "Check result value, expected: >0, got: %" SDL_PRIu64, result);

    /* Delay a bit longer and measure ticks and verify difference */
    SDL_Delay(testDelay);
    SDL_Log("Call to SDL_Delay(%d)", testDelay);
    result2 = SDL_GetTicks();
    SDL_Log("Call to SDL_GetTicks()");
    SDL_Log(result2 > 0, "Check result value, expected: >0, got: %" SDL_PRIu64, result2);
    difference = result2 - result;
    SDL_Log(difference > (testDelay - marginOfError), "Check difference, expected: >%d, got: %" SDL_PRIu64, testDelay - marginOfError, difference);
...
int main(int argc, char *argv[])
{
    ...
    SDL_Log("1 million iterations of ticktock now=%d, start=%d took %f ms\n",now, start, (double)((now - start) * 1000) / SDL_GetPerformanceFrequency());
